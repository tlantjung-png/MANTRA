...
from tests.test_tui import _make_app, grid_rows, wait_until
...
class D1TabSanitizationTest(unittest.TestCase):
    """Raw tabs must never reach the terminal (D1)."""
...
class D2FrameRecoveryTest(unittest.TestCase):
    """A failing render_frame must keep the repaint request armed (D2)."""
...
class D3SessionPanelUnitsTest(unittest.TestCase):
    """The session panel must slice entries by the row budget (D3)."""
...
class D4OffsetClampTest(unittest.TestCase):
    """Offset must be clamped after rewrap so the viewport is not blank (D4)."""
...
class D5PasteRoutingTest(unittest.TestCase):
    """A paste while a modal overlay is open must not reach the composer (D5)."""
