...
class _ConsoleSession:
    ...
    def wait_for_output_idle(self, timeout: float = 15.0, idle: float = 0.4) -> None:
        """Wait until the output stream stops growing for `idle` seconds.

        Event-driven replacement for fixed post-action sleeps: whatever
        the child was going to paint has painted (or the timeout
        expires). The stream is append-only, so its length is a
        ...
        while time.monotonic() < deadline:
            ...
            if size_now != size:
                ...
                return
        with self._lock:
            tail = "".join(self.output)[-1200:]
        raise AssertionError(
            f"output never went idle within {timeout}s; recent tail:\n{tail!r}"
        )

    def wait_composer_ready(self, timeout: float = 30.0) -> None:
        """Wait for the composer prompt to paint and startup to settle.

        Replaces the fixed post-boot sleep. The prompt row alone is not
        enough: the app can flush keys that arrived before its reader
        started, so ready means the composer is on screen AND the boot
        output burst has stopped growing.
        """
        self.wait_for("MANTRA >", timeout=timeout)
        self.wait_for_output_idle(timeout=timeout)

    def wait_for_count(self, needle: str, minimum: int, timeout: float = 15.0) -> int:
        """Wait until needle has painted at least `minimum` times.

        The stream is append-only, so re-renders (a popup re-opened, a
        message echoed into the transcript) show up as the occurrence
        count rising, never as old text vanishing.
        """
        deadline = time.monotonic() + timeout
        count = 0
        while time.monotonic() < deadline:
            count = self.text().count(needle)
            if count >= minimum:
                return count
            time.sleep(0.05)
        raise AssertionError(
            f"{needle!r} seen {count} times, wanted >= {minimum} within {timeout}s"
        )

    def type_and_wait_echo(self, text: str, timeout: float = 10.0) -> str:
        """Type text and wait until the composer has echoed it.

        The app reads stdin in chunks, so Enter sent in the same burst
        as the text can be consumed before the text is processed;
        waiting for the echo paces the following Enter like a fast
        human typist, independent of machine load.
        """
        base = len(self.text())
        self.send(text)
        deadline = time.monotonic() + timeout
        while time.monotonic() < deadline:
            seen = self.text()
            if text in seen[base:]:
                return seen
            time.sleep(0.05)
        raise AssertionError(f"typed text {text!r} never echoed within {timeout}s")
