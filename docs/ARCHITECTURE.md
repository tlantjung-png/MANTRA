...
Persistence is file-based and atomic. Settings, credentials, workflows, and session transcripts are written via uniquely named temporary files with restricted permissions and then moved into place, with quarantine handling for corrupted files under collision-free backup names and verified stale-lock handling for concurrent access. Background task logs are written to workspace-private or per-process private locations with restricted permissions and are bounded and pruned. Transcript ingest strips every escape sequence except color and weight in a single pass without printable stand-ins, so model output can never forge a kept sequence by echoing the sanitizer's internal markers.
