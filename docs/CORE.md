...
Durable memory entries carry lifecycle metadata. A planner classifies a new entry as a duplicate to skip, a replacement that marks an older entry superseded, or a fresh append, and filtered reads exclude superseded and stale entries from prompt injection.
