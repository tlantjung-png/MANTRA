...
The console is a single application-owned terminal surface, built the way reference terminal agents are built: every frame is composed into a full cell grid (character plus style per cell), diffed against the previously shown grid, and only the changed cells are written out. A dedicated reader thread translates raw terminal input into data events (keys, mouse, bracketed paste, real resize events on Windows); one presenter loop drains that queue fully before drawing, batches streaming model output, throttles frames, and arms an animation tick only while something is actually animating (the spinner, drag auto-scroll). Because nothing is ever painted at a stale absolute position by a background thread, a resize is simply a full reflow and interleaved output can never tear a frame. The screen is an alternate screen with a top information bar (workspace, model, approval mode, cache rate), a scrollable transcript viewport, a border row carrying the spinner and status text, and the prompt box at the bottom. Text selection is drawn by the application: drag over the transcript to select, release to copy (with the system clipboard as the first route and an escape-sequence fallback for remote sessions); the wheel and page keys scroll; during a turn the operator can keep typing, and the next prompt is queued and sent when the turn ends. Slash-command menus, endpoint/model pickers, secret entry and approval prompts are overlay cards rendered into the frame. The prompt box is closed with corners and a right wall so long input never escapes it. A scroll marker rides the border row while the viewport is scrolled up. The layout wraps long lines at word boundaries, counting wide characters double, and re-emits open style codes when a wrap lands inside a coloured word. Resize events re-wrap stored content and keep the same logical line at the top of the viewport.
